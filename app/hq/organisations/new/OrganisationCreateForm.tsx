...
import {
  createOrganisation,
  type CreateOrganisationActionState,
} from "./actions";
import { createOrganisationSchema } from "./schema";

const initialCreateOrganisationState: CreateOrganisationActionState = {
  message: "",
  fieldErrors: {},
};

export default function OrganisationCreateForm() {
  const [serverState, formAction] = useActionState(createOrganisation, initialCreateOrganisationState);
  ...
  const colorError = clientErrors.primaryColor ?? serverState.fieldErrors.primaryColor;
  const logoError = clientErrors.logoUrl ?? serverState.fieldErrors.logoUrl;
    ...
    <form action={formAction} onSubmit={onSubmit} className="space-y-6">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div>
          ...
          {colorError ? <p className="mt-2 text-xs font-medium text-rose-600">{colorError}</p> : null}
        </div>
...
function SubmitButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="inline-flex min-w-44 items-center justify-center rounded-lg bg-sky-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-sky-700 disabled:cursor-not-allowed disabled:bg-sky-300"
    >
      {pending ? "Creation en cours..." : "Creer l organisation"}
    </button>
  );
}
